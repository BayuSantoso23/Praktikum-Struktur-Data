...
class LinkedList:
    ...
    def addLast(self, data):
        newNode = _Node(data)  # Membuat node baru dengan data yang diberikan
        if self._head is None:  # Jika linked list kosong
            self._head = newNode
        else:
            curNode = self._head
            while curNode.next is not None:  # Traverse hingga node terakhir
                curNode = curNode.next
            curNode.next = newNode  # Hubungkan node terakhir ke node baru
        self._size += 1
